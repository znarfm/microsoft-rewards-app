...
- **Persistence**: read/write `SharedPreferences` directly (no persistence layer); store raw `TextEditingController` strings for count/delay. Defaults: count `'22'`, delay `'15'`, send_daily_reminder `false`, keep_screen_on `true`, show_login_reminder_popup `true`, reminder 19:00, theme_mode `system`, amoled_screen_off `false`.
